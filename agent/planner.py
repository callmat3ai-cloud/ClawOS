...
PLANNER_PROMPT = """You are the planning module of ClawOS, a powerful desktop AI assistant.
...
- Use composio_* tools for any app-level actions (email, calendar, social media, etc.)
...
def create_plan(
    ...
    if context:
        user_input += f"\n\nContext from memory:\n{context}"
    if composio_context:
        user_input += f"\n\nAVAILABLE APP INTEGRATIONS (use these as composio_<app>.<action>):\n{composio_context}"
