...
class ExecutionResult:
    ...
    def __init__(self, text: str, actions_used: list[str] | None = None,
                 action_results: list[str] | None = None):
        self.text = text
        self.actions_used = actions_used or []
...
class AgentExecutor:
    ...
    def __init__(self):
        self.actions_used_log: list[str] = []
    ...
    def execute(
        ...
        while True:
            ...
            if not steps:
                ...
                if speak: speak(msg)
                return ExecutionResult(text=msg)
            ...
            for step in steps:
                if cancel_flag and cancel_flag.is_set():
                    ...
                    return ExecutionResult(text="Task cancelled.")

                step_num = step.get("step", "?")
                tool     = step.get("tool", "generated_code")
                desc     = step.get("description", "")
                params   = step.get("parameters", {})
                actions_used.append(tool)
            ...
            if success:
                text = self._summarize(goal, completed_steps, speak)
                return ExecutionResult(text=text, actions_used=actions_used)

            if replan_attempts >= self.MAX_REPLAN_ATTEMPTS:
                msg = f"Task failed after {replan_attempts} replan attempts, sir."
                if speak: speak(msg)
                return ExecutionResult(text=msg, actions_used=actions_used)
